    ...
    public class TicketController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetOfficialReceiptInfo([FromBody] ORItem data)
        {
            ...
            if (compute.Contains("success"))
            {
                ...
                {
                    orinfo += $"[L]DISCOUNT({decimal.ToInt32(result.DiscountPercentage)}%)    :P {result.Discount}\n";
                }
                else
                {
                ...
                try
                {
                    ...
                    {
                        toExport.Add(StringGenerator.FormatLabelWithDoubleValue($"DISCOUNT ({decimal.ToInt32(result.DiscountPercentage)}%)", result.Discount));
                    }
                    else
                    {
        ...
        public async Task<IHttpActionResult> GetXReadingReport(int gateid, string srno)
        {
            ...
            bodyString += $"[L]CASH [R]{reading.CashPayment}\n";
        ...
        public async Task<IHttpActionResult> GetZReadingReport(int gateid, string srno, int userid)
        {
            ...
            bodyString += $"[L]================================\n";
            bodyString += $"[L]Beginning Reading: [R]{reading.PresentAccumulatedSales}\n";
            bodyString += $"[L]Ending Reading: [R]{reading.PreviousAccumulatedSales}\n";
